...
export function useCart() {
  ...
  const add = (productId: string, kilos: number, quality?: Quality) => setItems((old) => { const index = old.findIndex((i) => i.productId === productId && i.quality === quality); return index >= 0 ? old.map((i, n) => n === index ? { ...i, kilos: i.kilos + kilos } : i) : [...old, { productId, kilos, quality }] })
